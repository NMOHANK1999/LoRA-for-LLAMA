...
class ModelSampler:
    def __init__(self, out_dir, init_from="resume", device="cuda", max_new_tokens=5, temperature=1.0, top_k=200, ckpt_last = False):
    ...
    def predict_labels(self, dataset):
        ...
        for i,row in enumerate(dataset):
            ...
            ################################################## TODO: ##################################################
            truncated_response = pred_samples[i]['response'][len(prompt)-1:].strip()
            first_20_chars = truncated_response[:20] 
            if "positive" in first_20_chars:
                predicted_label = 1
            elif "negative" in first_20_chars: 
                predicted_label = 0
            else:
                predicted_label = -1
