...
class CustomDataLoader:
    ...
    def _add_instruction_finetuning(self, rec):
        '''rec is a dictionary with keys "text" and "label"
        '''
        # Convert label from 0/1 to "negative"/"positive" for better intuition during generation.  
        label = "positive" if rec["label"] == 1 else "negative"
        rec["instr_tuned_text"] = get_sentiment_prompt(rec["text"], label)
        return rec
